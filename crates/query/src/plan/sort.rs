...
use arrow::{
    ...
    datatypes::{DataType, Int32Type, Int64Type, Schema, UInt16Type, UInt32Type, UInt64Type}
};

use crate::{
    plan::key::{Key, PrimitiveKey, PrimitiveListKey},
    primitives::{schema_error, Name, SchemaError}
};

pub type Position = (usize, usize);

pub fn compute_order(record_batches: &Vec<RecordBatch>, key: &[Name]) -> Result<Vec<Position>, SchemaError> {
    if record_batches.len() == 0 {
        return Ok(vec![]);
    }

    let mut positions = Vec::with_capacity(record_batches.iter().map(|b| b.num_rows()).sum());
    ...
    match key.types.as_slice() {
        ...
        [DataType::Int32, DataType::Int32] => sort_by_2::<PrimitiveKey<Int32Type>, PrimitiveKey<Int32Type>>(
            &mut positions,
            record_batches,
            key.indexes[0],
            key.indexes[1]
        ...
        [DataType::Int32, DataType::Int32, DataType::List(field)] if field.data_type() == &DataType::Int32 => {
            sort_by_3::<PrimitiveKey<Int32Type>, PrimitiveKey<Int32Type>, PrimitiveListKey<Int32Type>>(
                &mut positions,
                record_batches,
                key.indexes[0],
                key.indexes[1],
        ...
        [DataType::Int32, DataType::Int32, DataType::List(field)] if field.data_type() == &DataType::UInt32 => {
            sort_by_3::<PrimitiveKey<Int32Type>, PrimitiveKey<Int32Type>, PrimitiveListKey<UInt32Type>>(
                &mut positions,
                record_batches,
                key.indexes[0],
                key.indexes[1],
        ...
        _ => {
            ...
            positions.sort_unstable_by(|a, b| column_list.compare_positions(*a, *b));
        }
    }
    ...
    fn new(schema: &Schema, key: &[Name]) -> Result<Self, SchemaError> {
        ...
        for &name in key.iter() {
            let (idx, field) = schema
                ...
                .ok_or_else(|| schema_error!("key column `{}` is not found in result", name))?;
            indexes.push(idx);
...
#[inline(never)]
fn sort_by_1<'a, K>(positions: &mut Vec<Position>, record_batches: &'a Vec<RecordBatch>, key_idx: usize)
where
    K: Cmp,
    K: From<&'a dyn Array>
{
    let comparators: Vec<K> = record_batches
        .iter()
        .map(|b| {
...
fn sort_by_2<'a, K1, K2>(
    ...
    K2: From<&'a dyn Array>
{
    let comparators: Vec<_> = record_batches
        .iter()
        .map(|b| {
...
fn sort_positions<C: Cmp>(positions: &mut Vec<(usize, usize)>, comparators: &Vec<C>) {
...
impl ColumnList {
    fn new(record_batches: &Vec<RecordBatch>, key: &TableKey) -> Result<Self, SchemaError> {
        let columns = key
            ...
            .cloned()
            .enumerate()
            .map(|(i, kix)| get_column(record_batches, kix, &key.types[i]).map_err(|err| err.at(key.names[i])))
...
fn get_column(
    ...
    match data_type {
        ...
        _ => Err(schema_error!("unsupported key column type - {}", data_type))
    }
}

pub fn make_pos_comparator(record_batches: &Vec<RecordBatch>, key: &[Name]) -> Result<impl PosCmp, SchemaError> {
    if record_batches.len() == 0 {
