use arrow::{
    array::{
        Array, AsArray, BinaryArray, BooleanArray, FixedSizeBinaryArray, GenericListArray, GenericStringArray,
...
pub fn extract_nulls(array: &dyn Array) -> Result<(Option<Box<dyn Array>>, Option<NullBuffer>), SchemaError> {
    ...
    match array.data_type() {
        ...
        DataType::Binary => {
            ...
            ok!(unsafe { BinaryArray::new_unchecked(offsets, values, None) }, nulls)
        }
        DataType::Utf8 => {
            let array = array.as_string::<i32>();
            let (offsets, values, nulls) = array.clone().into_parts();
            ok!(
                unsafe { GenericStringArray::<i32>::new_unchecked(offsets, values, None) },
                nulls
            )
        }
        DataType::List(_) => {
        ...
        DataType::Struct(_) => {
            ...
            ok!(unsafe { StructArray::new_unchecked(fields, columns, None) }, nulls)
        }
        _ => Err(SchemaError::new(format!(
            "unsupported arrow type - {}",
            array.data_type()
