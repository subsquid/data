...
use crate::{
    ...
    scan::{
        ...
        arrow::IntoArrowScalar,
        row_predicate::{AndPredicate, ColumnPredicate, OrPredicate, RowPredicateRef},
        IntoArrowArray
    }
};
...
macro_rules! make_column_predicate {
    ($col:expr, $arr_predicate:expr) => {
        Arc::new(ColumnPredicate::new($col, Arc::new($arr_predicate)))
    };
}
...
pub fn col_in_list<L: IntoArrowArray>(name: Name, values: L) -> RowPredicateRef {
    ...
    match values.len() {
        ...
        _ => {
            make_column_predicate!(name, array_predicate::InList::new(values))
        }
    }
}
...
pub fn bloom_filter<L>(name: Name, bytes_size: usize, num_hashes: usize, values: L) -> RowPredicateRef
where
    L: IntoIterator<Item: Hash>
{
    let array_pred = array_predicate::or(
        values
            .into_iter()
            .map(|val| Arc::new(array_predicate::BloomFilter::new(bytes_size, num_hashes, val)) as ArrayPredicateRef)
            .collect()
    );
...
pub fn and(predicates: Vec<RowPredicateRef>) -> RowPredicateRef {
    if predicates.len() == 1 {
        ...
        Arc::new(AndPredicate::new(predicates))
    }
}
...
pub fn or(predicates: Vec<RowPredicateRef>) -> RowPredicateRef {
    if predicates.len() == 1 {
        ...
        Arc::new(OrPredicate::new(predicates))
    }
}
