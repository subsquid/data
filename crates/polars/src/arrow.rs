...
pub fn array_series(name: &str, arr: &dyn Array) -> anyhow::Result<Series> {
    let s = Series::from_arrow(name.into(), Box::<dyn polars_arrow::array::Array>::from(arr))?;
    Ok(s)
}
...
pub fn record_batch_to_polars_df(batch: &RecordBatch) -> anyhow::Result<DataFrame> {
    ...
    for (i, column) in batch.columns().iter().enumerate() {
        columns.push(array_series(schema.fields().get(i).unwrap().name(), column)?);
    }
...
pub fn sort_record_batch(record_batch: &RecordBatch, by: Vec<String>) -> anyhow::Result<RecordBatch> {
    ...
    let sorted_df = df.sort(by, SortMultipleOptions::default().with_multithreaded(false))?;

    let schema = record_batch.schema();

    let columns: Vec<ArrayRef> = sorted_df
        .iter()
        .enumerate()
        .map(|(i, s)| {
