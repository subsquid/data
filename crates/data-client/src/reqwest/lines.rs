...
impl LineStream {
    ...
    fn check_line(&mut self) -> Option<Bytes> {
            ...
            .position(|b| *b == b'\n')
        {
            ...
            self.line
                .advance(if self.line.get(2).copied() == Some(b'\r') {
                    2
                } else {
                    1
                });
        ...
        loop {
            ...
            if let Some(inner) = this.inner.as_mut() {
                match Pin::new(inner).poll_next(cx) {
                    Poll::Ready(None) => {
                        this.inner = None;
                        return Poll::Ready(Ok(this.take_final_line()).transpose());
                    }
                    Poll::Ready(Some(Ok(bytes))) => this.line.extend_from_slice(&bytes),
