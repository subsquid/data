...
use sqd_array::{
    io::file::{ArrayFile, ArrayFileReader, ArrayFileWriter},
    reader::ArrayReader,
    slice::{AnyTableSlice, Slice},
    writer::ArrayWriter
};
...
impl TableFileWriter {
    ...
    pub fn finish(self) -> anyhow::Result<TableFile> {
        ...
        let readers = columns.iter().map(|c| c.read()).collect::<Result<Vec<_>, _>>()?;

        Ok(TableFile { columns, readers })
    }
}
