...
use crate::{
    offsets::Offsets,
    slice::{nullmask::NullmaskSlice, AnyListItem, AnySlice, ListSlice, PrimitiveSlice, Slice},
    writer::{ArrayWriter, NativeWriter, OffsetsWriter}
};
...
pub fn common_item_index_type(a: &DataType, b: &DataType) -> Option<DataType> {
    match (a, b) {
        (DataType::List(ai), DataType::List(bi)) => common_index(ai.data_type(), bi.data_type()).map(|t| {
            let field = Field::new(ai.name(), t, ai.is_nullable() || bi.is_nullable());
...
fn cast_primitive<S, T>(src: &PrimitiveSlice<'_, S>, dst: &mut impl ArrayWriter) -> anyhow::Result<()>
where
    S: ArrowNativeType,
    T: ArrowNativeType,
    T: TryFrom<S, Error: Debug>
{
    ...
    dst.native(1)
        .write_iter(src.values().iter().map(|v| match T::try_from(*v) {
            Ok(value) => value,
            Err(err) => {
    ...
    T: TryFrom<S, Error: Debug>
{
    ...
    cast_primitive::<S, T>(&items.slice(item_range.start, item_range.len()), &mut dst.shift(2))
}
