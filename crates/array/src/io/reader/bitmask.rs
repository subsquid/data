...
use crate::{io::reader::byte_reader::ByteReader, reader::BitmaskReader, writer::BitmaskWriter};
...
impl<R: ByteReader> BitmaskIOReader<R> {
    pub fn new_unchecked(byte_reader: R, len: usize) -> Self {
        Self { byte_reader, len }
    }

    pub fn new(mut byte_reader: R) -> anyhow::Result<Self> {
        let (byte_len, bit_len) = Self::read_length(&mut byte_reader).context("failed to read bitmask length")?;
        ...
        Ok(Self::new_unchecked(byte_reader, bit_len))
    }
