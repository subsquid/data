...
impl<'a> DenseReader<'a> {
    pub fn new(data: &'a [u8]) -> anyhow::Result<Self> {
        ensure!(data.len() >= USIZE, "dense file must have at least {} bytes", USIZE);

        let num_buffers = u32::from_le_bytes(data[data.len() - USIZE..].try_into()?) as usize;
        ...
        let buffer_lengths: Vec<usize> = (0..num_buffers)
            .map(|i| {
                let beg = data.len() - min_byte_size + i * USIZE;
                let end = beg + USIZE;
                u32::from_le_bytes(data[beg..end].try_into().unwrap()) as usize
            })
            .collect();
