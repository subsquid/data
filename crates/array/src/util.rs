...
#[inline]
pub fn validate_offsets<I: Ord + Default + Copy>(offsets: &[I], mut prev: I) -> Result<(), &'static str> {
    if offsets.len() == 0 {
...
pub mod bit_tools {
    ...
    use arrow_buffer::{bit_chunk_iterator::UnalignedBitChunk, bit_util};
...
pub fn get_offset_position<I: Ord + Copy>(offsets: &[I], index: I, first_to_try: usize) -> usize {
    ...
    if beg <= index {
        if index < offsets[first_to_try + 1] {
            ...
            first_to_try + bisect_offsets(&offsets[first_to_try..], index).expect("index is out of bounds")
        }
    } else {
        bisect_offsets(&offsets[0..first_to_try + 1], index).expect("index is out of bounds")
    }
}
...
pub fn build_field_offsets(start_pos: usize, fields: &Fields) -> Vec<usize> {
    build_offsets(start_pos, fields.iter().map(|f| get_num_buffers(f.data_type())))
}
...
pub fn get_num_buffers(data_type: &DataType) -> usize {
    match data_type {
        ...
        DataType::Binary | DataType::Utf8 => 3,
        DataType::FixedSizeBinary(_) => 2,
        DataType::List(f) => 2 + get_num_buffers(f.data_type()),
        DataType::FixedSizeList(f, _) => 1 + get_num_buffers(f.data_type()),
        DataType::Struct(fields) => 1 + fields.iter().map(|f| get_num_buffers(f.data_type())).sum::<usize>(),
