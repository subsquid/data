...
pub enum AnySlice<'a> {
    ...
    Address16(ListSlice<'a, &'a [u16]>),
    Address32(ListSlice<'a, &'a [u32]>),
...
impl <'a> Slice for AnySlice<'a> {
    fn num_buffers(&self) -> usize {
        match self {
            ...
            AnySlice::Address16(s) => s.num_buffers(),
            AnySlice::Address32(s) => s.num_buffers(),
    ...
    fn len(&self) -> usize {
        match self {
            ...
            AnySlice::Address16(s) => s.len(),
            AnySlice::Address32(s) => s.len(),
    ...
    fn slice(&self, offset: usize, len: usize) -> Self {
        match self {
            ...
            AnySlice::Address16(s) => AnySlice::Address16(s.slice(offset, len)),
            AnySlice::Address32(s) => AnySlice::Address32(s.slice(offset, len)),
    ...
    fn write(&self, dst: &mut impl ArrayWriter) -> anyhow::Result<()> {
        match self {
            ...
            AnySlice::Address16(s) => s.write(dst),
            AnySlice::Address32(s) => s.write(dst),
    ...
    fn write_range(&self, dst: &mut impl ArrayWriter, range: Range<usize>) -> anyhow::Result<()> {
        match self {
            ...
            AnySlice::Address16(s) => s.write_range(dst, range),
            AnySlice::Address32(s) => s.write_range(dst, range),
    ...
    fn write_ranges(&self, dst: &mut impl ArrayWriter, ranges: &mut impl RangeList) -> anyhow::Result<()> {
        match self {
            ...
            AnySlice::Address16(s) => s.write_ranges(dst, ranges),
            AnySlice::Address32(s) => s.write_ranges(dst, ranges),
    ...
    fn write_indexes(&self, dst: &mut impl ArrayWriter, indexes: impl IntoIterator<Item=usize, IntoIter: Clone>) -> anyhow::Result<()> {
        match self {
            ...
            AnySlice::Address16(s) => s.write_indexes(dst, indexes),
            AnySlice::Address32(s) => s.write_indexes(dst, indexes),
