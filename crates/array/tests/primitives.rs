...
use sqd_array::{
    builder::offsets::OffsetsBuilder,
    io::{
        reader::{IOByteReader, OffsetsIOReader},
        writer::OffsetsIOWriter
    },
    offsets::Offsets,
    reader::OffsetsReader,
    writer::OffsetsWriter
};
...
fn test_io_offsets_read_all() -> anyhow::Result<()> {
    ...
    let mut reader = OffsetsIOReader::new(IOByteReader::new(offsets.to_byte_slice().len(), cursor))?;
