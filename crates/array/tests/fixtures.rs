...
fn test_write_reversed_slice_to_builder() -> anyhow::Result<()> {
    ...
    let result = {
        ...
        src.as_slice()
            .write_indexes(&mut builder, indexes.values().iter().map(|i| *i as usize))?;
...
fn test_in_memory_sort() -> anyhow::Result<()> {
    ...
    let result = sort_record_batch(
        &src,
        ["program_id", "block_number", "transaction_index", "instruction_address"]
    )?;
