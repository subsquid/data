...
impl Fs for S3Fs {
    // fn abs(&self, segments: &[&str]) -> String {
    //     format!("s3://{}", self.abs_path(segments))
    // }
    ...
    // fn transact(&self, dest_dir: &str) -> Box<dyn Fs> {
    //     let path = self.abs_path(&[dest_dir]);
    //     Box::new(S3Fs::new(&path, "us-east-1", "access_key", "secret_key"))
    // }
...
pub fn create_fs(value: &str) -> anyhow::Result<Box<dyn Fs>> {
    let url = Url::parse(value).map_err(|_| anyhow::anyhow!("unsupported url - {}", value))?;
    match url.scheme() {
        "s3" => {
        ...
        _ => anyhow::bail!("unsupported filesystem - {}", url.scheme()),
    }
}
