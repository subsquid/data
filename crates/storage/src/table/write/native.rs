...
use sqd_array::{index::RangeList, writer::NativeWriter};

use crate::table::write::page::PageWriter;

pub static PAGE_SIZE: LazyLock<RwLock<usize>> = LazyLock::new(|| RwLock::new(64 * 1024));
...
impl<P: PageWriter> NativePageWriter<P> {
    ...
    pub fn finish(mut self) -> anyhow::Result<P> {
        ensure!(self.buffer.len() % self.item_size == 0, "got partially written item");
        ...
        if byte_end > byte_offset {
            self.page_writer
                .write_page((byte_end - byte_offset) / self.item_size, &self.buffer[byte_offset..])?;
        }
