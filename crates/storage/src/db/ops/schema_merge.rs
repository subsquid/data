...
use sqd_array::{item_index_cast::common_item_index_type, schema_metadata::get_sort_key, schema_patch::SchemaPatch};
...
pub fn merge_schema(base: &mut SchemaPatch, schema: &Schema) -> anyhow::Result<()> {
    ...
    for new_field in schema.fields().iter() {
        let (bi, bf) = base
            ...
            .ok_or_else(|| anyhow!("field `{}` does not exist in the base schema", new_field.name()))?;

        if &bf == new_field {
            continue;
        }

        let new_field = merge_fields(&bf, new_field).ok_or_else(|| {
            anyhow!(
...
fn merge_fields(a: &Field, b: &Field) -> Option<FieldRef> {
    ...
    data_type.map(|data_type| {
        let field = Field::new(a.name(), data_type, a.is_nullable() || b.is_nullable());
        Arc::new(field)
    })
}
