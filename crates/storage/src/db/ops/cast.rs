...
impl<S: ArrayReader> IndexCastReader<S> {
    ...
    pub fn read_slice(&mut self, dst: &mut impl ArrayWriter, mut offset: usize, mut len: usize) -> anyhow::Result<()> {
        while len > 0 {
...
impl<R: ArrayReader> MaybeCastedReader<R> {
    ...
    pub fn read_slice(&mut self, dst: &mut impl ArrayWriter, offset: usize, len: usize) -> anyhow::Result<()> {
        match self {
