use crate::{
    db::db::{RocksDB, RocksWriteBatch, CF_DELETED_TABLES, CF_DIRTY_TABLES, CF_TABLES},
    kv::KvReadCursor,
...
fn delete_table(db: &RocksDB, table_id: &[u8]) -> anyhow::Result<()> {
    ...
    list_keys(&mut cursor, start, end, |key| batch.delete_cf(cf_tables, key))?;
...
fn list_keys(cursor: &mut impl KvReadCursor, from: &[u8], to: &[u8], mut cb: impl FnMut(&[u8])) -> anyhow::Result<()> {
    cursor.seek(from)?;
...
fn for_each_key(cursor: &mut impl KvReadCursor, mut cb: impl FnMut(&[u8]) -> anyhow::Result<()>) -> anyhow::Result<()> {
    cursor.seek_first()?;
