//! [`MockDB`] -- a scratch temp-dir `Database` for driving the table cleanup lifecycle in
//! tests: logical purge (Phase 1, point deletes) + physical SST-file unlink below the live
//! watermark. It hides the fiddly setup (auto-compaction off, small write buffers, UUIDv7
//! id ordering, flush/compact to the bottom level).
...
/// A table [`MockDB`] built: enough to delete it, read it back, and reason about its
/// watermark position (`id`).
pub struct Table {
    pub chunk: Chunk,
    pub id: TableId,
...
/// A scratch `Database` wired for deterministic cleanup/reclaim tests: auto-compaction off
/// (only explicit `compact`/unlink move data) and small write buffers (so modest row counts
/// still produce real SST files).
///
/// Invariant: tables are created with strictly increasing ids in call order, so "created
/// first" == "smaller id". Tests rely on this to place an orphan/older table below a live
/// one without reading raw ids.
pub struct MockDB {
    db: Database,
    ds: DatasetId,
    schema: Arc<Schema>,
...
impl MockDB {
    ...
    /// Like [`MockDB::new`] but with the block cache disabled, so reads must hit the SST
    /// files: once unlinked, the data is genuinely gone, never served from cache.
    ...
    /// Build (and `finish`) a table of `rows` rows over the next free block range.
    /// `finish` already persists the dirty marker and the table data; the caller decides
    /// whether to commit the chunk.
    fn build_table(&mut self, rows: usize) -> (Chunk, Vec<(u32, u32)>, TableId) {
        // Advance the UUIDv7 timestamp, keeping creation order == id order.
    ...
    /// Build a committed table: inserting its chunk removes the dirty marker.
    ...
    /// Build a table but never commit its chunk, so nothing removes its dirty marker --
    /// exactly the orphan a crash-before-commit leaves behind.
    ...
    /// Delete the whole dataset, which runs Phase 1 synchronously.
    ...
    /// Land the table data in the bottom level, so a later reclaim is a real file unlink
    /// (`DeleteFilesInRange` skips L0). In production the dead data lives there too.
