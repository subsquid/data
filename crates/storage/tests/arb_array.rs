...
use arrow::{
    array::{
        Array, ArrayRef, BinaryArray, BooleanArray, FixedSizeBinaryArray, Int16Array, Int32Array, Int64Array,
        Int8Array, ListArray, StringArray, StructArray, TimestampSecondArray, UInt16Array, UInt32Array, UInt64Array,
        UInt8Array
    },
    datatypes::{DataType, Field, Int32Type}
};
use arrow_buffer::BooleanBuffer;
use proptest::{collection::SizeRange, prelude::*, string::string_regex};
...
pub fn fixed_size_binary(len: impl Into<SizeRange> + Clone, size: usize) -> impl Strategy<Value = ArrayRef> {
    prop::collection::vec(prop::collection::vec(any::<u8>(), size), len.clone()).prop_map(move |vecs| {
