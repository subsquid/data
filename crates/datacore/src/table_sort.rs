...
#[cfg(test)]
mod test {
    use crate::table_sort::TableSorter;
    ...
    fn sort_ethereum_transactions() -> anyhow::Result<()> {
        let mut src_reader = open("fixtures/ethereum-transactions.parquet")?.build()?;
        ...
        let mut ref_reader = open("fixtures/ethereum-transactions.sorted.parquet")?
            .with_batch_size(33)
            .build()?;
    ...
    fn open(path: &str) -> anyhow::Result<ParquetRecordBatchReaderBuilder<File>> {
