...
impl<const N: usize> SID<N> {
    ...
    pub fn as_str(&self) -> &str {
        std::str::from_utf8(if let Some(end) = self.bytes.iter().position(|b| *b == 0) {
            &self.bytes[0..end]
        } else {
            &self.bytes
        })
...
impl<const N: usize> borsh::BorshDeserialize for SID<N> {
    fn deserialize(buf: &mut &[u8]) -> std::io::Result<Self> {
        ...
        Self::try_new(bytes).map_err(|err| std::io::Error::new(std::io::ErrorKind::InvalidData, err))
    }
            ...
            E: serde::de::Error
        {
            SID::try_from(v).map_err(|msg| {
                serde::de::Error::custom(format!(
                    "failed to deserialize `{}` as {}-byte short id string: {}",
                    v, N, msg
                ))
            })
        }
