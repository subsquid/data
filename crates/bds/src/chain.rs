...
impl <B: Block> Chain<B> {
    ...
    pub fn stored_head(&self) -> Option<BlockPtr<'_>> {
    ...
    pub fn head(&self) -> Option<BlockPtr<'_>> {
        self.chain.back().map(|b| b.ptr())
    }

    pub fn compute_fork_base(&self, prev: &[BlockRef]) -> anyhow::Result<Option<BlockPtr<'_>>> {
